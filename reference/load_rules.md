...
Loads the file-context, code-context, and pattern rules, and the
...
- code_contexts:
  ...
  Data frame with columns `name`, `version`, `type`, `message`, `xpath`.

- patterns:

  Data frame with columns `name`, `version`, `type`, `message`, `attck`,
  `xpath`.
...
- phases:
  ...
  file- and code-context rule, plus the computed contexts `"Top-level"`
  and `"Other"`.
