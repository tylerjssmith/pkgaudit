...
A directory, unlike a tarball, has no single canonical byte stream to
hash. The manifest is returned alongside the hash so two disagreeing
scans can be diffed to find which file differs.

A directory hash is a weaker provenance claim than a tarball hash: its
scope depends on `exclude`, and empty directories and symlinks are not
represented, so trees differing only in those hash identically. For
vetting untrusted code, prefer hashing the tarball.
