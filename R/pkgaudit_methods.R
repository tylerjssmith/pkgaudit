...
#'   of five summary data frames, the scan `metadata`, the recorded `path`, and
#'   the recorded `phase` -- the phases the frames were filtered to, or `NULL`
#'   for an unfiltered summary. `print()` reads it to head the report with the
#'   phases asked for, so an empty section is not mistaken for a clean scan.
#'   \describe{
#'     \item{file_contexts}{`file_context`: each file context found, once.}
#'     \item{patterns}{`phase`, `rule`, `n`, `attck`: how often each pattern rule
#'       matched, split by the phase its code executes in.}
#'     \item{matches}{`phase`, `rule`, `n`, `attck`: as `patterns`, across the
#'       shell and Make-like files.}
#'     \item{coverage}{`status`, `top_level`, `type`, `files`, `lines`: how much
#'       of the package was read, grouped by where the files sit and what kind
#'       they are. `type` is what a file was read as, or its extension.}
#'     \item{errors}{`step`, `file_context`, `rule`, `error`: the object's
#'       `errors` rows, renamed for display.}
#'   }
...
#' subset afterwards. A filtered report names its phases in the header, so it
#' cannot be mistaken for a full scan.
#'
...
# Count findings by the phase they execute in, one row per phase and rule,
# carrying each rule's ATT&CK labels. Shared by the two findings frames that
...
# the same scan reads the same wherever it is run.
.summarize_findings <- function(findings, phase = NULL) {
  empty <- data.frame(
    phase = character(0L), rule = character(0L),
