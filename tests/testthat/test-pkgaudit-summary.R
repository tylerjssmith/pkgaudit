...
rich_obj <- function(errors = .empty_errors(), metadata = good_metadata()) {
  ...
  fc[1L, ] <- list("src/Makevars", "src/Makevars", "m")
  fc[2L, ] <- list("configure",    "configure",    "m")
  fc[3L, ] <- list("src/Makevars", "src/Makevars", "m")

  cc <- .empty_code_contexts()
  cc[1L, ] <- list("onload_code",   "R/zzz.R", 1L, 1L, "m")
  cc[2L, ] <- list("onattach_code", "R/zzz.R", 9L, 1L, "m")
  cc[3L, ] <- list("onload_code",   "R/aaa.R", 4L, 1L, "m")

  pt <- .empty_patterns()
  pt[1L, ] <- list("source_pattern", "R/zzz.R", 1L, 1L, "m", "T1059 T1195.002", "onload_code")
  pt[2L, ] <- list("rcurl_pattern",  "R/zzz.R", 2L, 1L, "m", "T1041 T1195.002", "onload_code")
  pt[3L, ] <- list("source_pattern", "R/aaa.R", 3L, 1L, "m", "T1059 T1195.002", "Top-level")
  pt[4L, ] <- list("source_pattern", "R/aaa.R", 7L, 1L, "m", "T1059 T1195.002", "Other")
...
test_that("summary.pkgaudit() returns a summary.pkgaudit object", {
  ...
  expect_named(s, c("file_contexts", "code_contexts", "patterns", "errors",
                    "metadata", "path"))
...
test_that("summary.pkgaudit() lists each context once, in the order first seen", {
  ...
  expect_equal(s$code_contexts$code_context, c("onload_code", "onattach_code"))
})

test_that("summary.pkgaudit() counts patterns by name and carries their attck", {
  s <- summary(rich_obj())
  expect_named(s$patterns, c("pattern", "occurrences", "attck"))
  # Sorted by pattern name, not by the order encountered.
  expect_equal(s$patterns$pattern,     c("rcurl_pattern", "source_pattern"))
...
test_that("print.summary.pkgaudit() writes every section in order", {
  ...
  expect_equal(sub(" -+$", "", headers),
               c("--- pkgaudit Summary", "--- File Contexts", "--- Code Contexts",
...
test_that("print.summary.pkgaudit() renders the finding tables", {
  ...
  expect_true(any(grepl("^pattern +occurrences attck$", lines)))
