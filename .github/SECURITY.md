...
pkgaudit analyses packages without executing them, and reads from but does not
write to disk with one exception: `audit_tarball()` extracts the tarball to a 
temporary directory (`temp_dir`, default `tempdir()`) and removes it when 
finished.
...
cleartext. If the package under audit may contain sensitive material, or you
...
Directory scans (`audit_package()`) and rule loading (`load_rules()`) do not 
write to disk.
