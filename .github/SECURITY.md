...
Untrusted tarballs are validated fail-closed *before* extraction: non-gzip
compression (bzip2, xz, zstd, compress — judged by magic bytes, not filename),
link entries, GNU long-name and PAX typeflags, path traversal, absolute and
drive-qualified paths, paths containing backslashes or control characters,
empty paths, decompression bombs (entry-count, size, and expansion-ratio caps),
