...
- **exportable** – C, C++, Fortran, Rust, Python, JavaScript, and
  vignette chunks in those languages. Not read, but
...
code that executes on [`library()`](https://rdrr.io/r/base/library.html)
...
`untrustedpkg` has an `.onLoad()` hook containing a
[`system()`](https://rdrr.io/r/base/system.html) call, which runs
...
runs at no phase because it runs only when someone calls it; and a
`configure` script containing `curl`, which runs when the package is
built, checked, or installed from source. Code that runs without being
...
can. Both are covered in [Getting Started with
...
database of rules shipped at `inst/db/rules.db`.
[`load_rules()`](https://tylerjssmith.github.io/pkgaudit/reference/load_rules.md)
verifies the database against its bundled `.sha256` sidecar on every
call and refuses to load a modified one. To check an installed copy
