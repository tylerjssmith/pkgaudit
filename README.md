...
- **parsed**: R is parsed for security-relevant patterns like `system()`
  ...
  `demo/`, `exec/`, `tests/`, `tools/`, `inst/tests/`, `inst/tinytest/`,
  and `inst/unitTests/`; examples and `\Sexpr{}` macros in `.Rd` files;
  R chunks in R Markdown, Quarto, Sweave, and R.rsp vignettes; and
...
- **exportable**: C, C++, Objective-C, Fortran, Rust, Java, Python,
  Perl, Ruby, Lua, and JavaScript source files (TypeScript is read as
  JavaScript), and vignette chunks written in those languages, are not
  read, but `export_unscanned()` can write them out for tools like
  Semgrep, blank-padded so that a finding’s line number still points
...
so code that executes on `library()` is distinguishable from code that
runs only when someone calls it. A finding that runs at no phase is
reported under `none`.
...
By contrast, `download.file()` in `R/fetch.R` sits in a regular function
body, so it is not known to run automatically and is reported under
`none`.
