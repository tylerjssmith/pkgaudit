...
checks whether the MD5 hash claimed by the mirror matches the MD5 hash of the
...
We assert that every available package was downloaded, that no extra package was
downloaded, and that the MD5 hashes claimed by the mirror match the hashes of
the downloaded files.

```{r verify_downloads}
# downloaded packages match available packages
expected = paste0(available$Package, "_", available$Version, ".tar.gz")
...
`version` columns to the data frames to distinguish the results.
